...
export function getLocalChaptersByBook(bookId) {
  try {
    // Make sure chaptersData is defined and imported
    const chaptersData = require('../data/chapters.json');
    
    // Get chapters for the book - try both string and number keys
    const chapters = chaptersData[bookId] || chaptersData[parseInt(bookId)] || [];
    
    return chapters.map(ch => ({
      ...ch,
      // Ensure all expected properties exist with fallbacks
      name_en: ch.name_en || ch.chapter_title || ch.title || ch.name || `Chapter ${ch.id}`,
      title: ch.title || ch.name_en || ch.chapter_title || ch.name || `Chapter ${ch.id}`,
      // Add other properties that might be expected by components
      name: ch.name || ch.name_en || ch.chapter_title || ch.title || `Chapter ${ch.id}`,
      chapter_title: ch.chapter_title || ch.name_en || ch.title || ch.name || `Chapter ${ch.id}`
    }));
  } catch (err) {
    console.error('Error in getLocalChaptersByBook:', err);
    return [];
  }
}
